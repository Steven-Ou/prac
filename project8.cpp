...
int main(){
    ...
    while(tower[1].size()<n+1){
        ...
        if(n%2 == 1){//checks if the ring is odd:
            ...
            //Update the candit ring to the smallest ring from the tower
            candit = tower[from].back();
            //Determine the destination tower for the next move
        ...
        //choose the next tower with the smallest top ring that is smallest than candit
            ...
            if(candit<tower[(from+2)%3].back()){
                to=(from+2)%3;//Move to the next tower.
            }else{
                to =(from+1)%3;//Move to the next other side tower.
            }
        }
    }
    return 0;
